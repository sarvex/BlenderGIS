...
class CAMERA_OT_add_georender_cam(bpy.types.Operator):
	...
	def execute(self, context):#every times operator redo options are modified
		...
		if (
			...
			or len(objs) > 2
			or len(objs) == 1
			and objs[0].type != 'MESH'
			or len(objs) == 2
			and {objs[0].type, objs[1].type} != {'MESH', 'CAMERA'}
		):
...
def register():
	try:
		...
		log.warning(
			f'{CAMERA_OT_add_georender_cam} is already registered, now unregister and retry... '
		)
		unregister()
		bpy.utils.register_class(CAMERA_OT_add_georender_cam)
